...
// 读取词典
const DICT_FILE = path.join(__dirname, '..', 'references', 'subtitle-dictionary.txt');
let dictionary = [];
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/api/save-srt') {
    const srt = generateSRT(subtitles);
    const srtPath = './3_输出/' + path.parse(VIDEO_PATH).name + '.srt';
  ...
  if (req.method === 'POST' && req.url === '/api/burn') {
    ...
    req.on('end', () => {
      try {
        ...
        const baseName = path.parse(VIDEO_PATH).name;
