...
- 如果用户选择火山引擎，或者当前 `VOLCENGINE_API_KEY` 为空，必须显式展示 API Key 获取的详细说明链接 `https://my.feishu.cn/wiki/Gh0MwxHePidsYfkIx7zcvJQynqc?from=from_copylink`，不能只给控制台入口；然后再写入 `VOLCENGINE_API_KEY` 与 `ASR_ENGINE=volcengine`。
